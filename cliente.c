...
int clienteTCP(char *servidor, const char *fichero);
int clienteUDP(char *servidor, const char *fichero);

int main(int argc, char const *argv[])
{
	char servidor[TAM_BUFFER];
	if (argc != 4)
	{
	...
	else if (strcmp(argv[2], "TCP") == 0)
	{
		...
		return clienteTCP(servidor, nombreArchivo);
	}
	else if (strcmp(argv[2], "UDP") == 0)
	{
		const char *nombreArchivo = argv[2];
		return clienteUDP(servidor, nombreArchivo);
	}
}
...
int clienteTCP(char *servidor, const char *fichero)
{
	...
	if (getaddrinfo(servidor, NULL, &hints, &resultados) != 0)
	{
